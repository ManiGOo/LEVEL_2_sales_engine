...
from app.services import lead_research_service
...
async def research_leads(
    ...
    return await lead_research_service.research_leads(
        req.company_keys,
...
async def get_lead_status(user=Depends(get_current_user)):
    return await lead_research_service.get_lead_status()


@router.get("/{company_key}")
async def get_lead_detail(company_key: str, user=Depends(get_current_user)):
    return await lead_research_service.get_lead_detail(company_key)
